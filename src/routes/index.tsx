...
import { CLINIC, formatHoursDisplay, isOpenNow } from "@/lib/clinic";
...
// Small live badge — computed on the client from the actual clock, re-checked every minute.
function OpenStatusBadge() {
  ...
  if (open === null) return null; // avoid a server/client mismatch flash
  return (
    <span
      className={cn(
    ...
    >
      ...
      {open ? "Open right now" : "Closed right now"}
    </span>
  );
}
